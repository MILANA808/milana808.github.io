/**
 * AKSI Resonance Composer v2.1 — русское offline-ядро + саморазвитие
 * Вопрос → намерение → доказательства → генерация → ADIA → рост
...
(function (global) {
  ...
  function intentOf(q) {
    ...
    if (/^(привет|здравствуй|hello|hi)([\s!.,]|$)/.test(s)) return "greet";
  ...
  function cleanBody(text) {
    ...
    text = text.replace(/\n?·[^\n]*$/gm, "").trim();
    text = text.replace(/\n?· ядро:[^\n]*/gi, "").trim();
    var lead = /^(Сформулирую так:|По сути:|Краткое определение:|Кратко:|Причина такая\.|Причина в следующем\.|Это связано с тем, что|Логика такая:|Как это работает:|Пошагово:|Механика такая:|Опираясь на локальные факты[^:]*:|Практическая опора:|Сравнение по доступным данным:|Если сопоставить:|Соберу ответ так:|Исходя из локального ядра:)\s*/i;
    for (var n = 0; n < 4; n++) text = text.replace(lead, "");
  ...
  function gatherEvidence(q, limit) {
    ...
    pool.forEach(function (e) { if (e.src === "grow" || e.src === "teach") e.score = (e.score || 0) + 0.08; });
  ...
  function uniqSentences(parts) {
    var out = [], seen = {}, i, p, key, j, skip;
    for (i = 0; i < parts.length; i++) {
      p = cleanBody(parts[i] || "");
      if (!p || p.length < 8) continue;
      if (!/[.!?…]$/.test(p)) p += ".";
      key = p.slice(0, 48).toLowerCase();
      if (seen[key]) continue;
      skip = false;
      for (j = 0; j < out.length; j++) {
        var a = out[j].toLowerCase(), b = p.toLowerCase();
        if (a.indexOf(b.slice(0, Math.min(40, b.length))) !== -1 || b.indexOf(a.slice(0, Math.min(40, a.length))) !== -1) { skip = true; break; }
      }
      if (skip) continue;
      seen[key] = 1;
      out.push(p);
  ...
  function generateAnswer(q, intent, ev, conf) {
    ...
    if (intent === "greet") return "Привет. Я АКСИ — локальное ядро с саморазвитием. Отвечаю без сети, учусь на этом устройстве через «запомни:» и обратную связь.";
    if (intent === "identity") return "Я АКСИ. Ядро: Resonance Composer + Neuro + ADIA. Собираю ответ из локальных знаний и могу записать его в рост ядра. На компьютере с WebGPU доступна нейросеть. Контакт: aksilove@internet.ru.";
    if (intent === "growth_status") {
      var st = loadStats(), g = loadGrow();
      return "Статус ядра: фактов в росте " + g.length + " из " + MAX_GROW + ", генераций " + (st.generations || 0) + ", обучений " + (st.teaches || 0) + ", подкреплений " + (st.reinforce || 0) + ". Саморазвитие только на этом устройстве.";
    }
    if (!sents.length || conf < 0.18) return "Пока мало опоры в ядре по этой теме. Напишите «запомни: факт» — встрою в рост. Или загрузите файл. Я не выдумываю факты без основания.";
    var openers = {
      define: ["Сформулирую так:", "По сути:", "Кратко:"],
      why: ["Причина такая.", "Это связано с тем, что", "Логика такая:"],
    ...
    if (conf < 0.45) tail += " Уверенность средняя — можно уточнить вопрос или добавить факт через «запомни:».";
    return ((opener ? opener + " " : "") + body + tail).replace(/\s+/g, " ").replace(/\s+\./g, ".").trim();
  }
  function growAbsorb(q, answer, conf, src) {
    var body = cleanBody(answer);
    if (!body || conf < 0.38 || body.length < 24) return false;
    if (/мало опоры|не выдумываю|Пустой факт/i.test(body)) return false;
    var grow = loadGrow();
    var entry = { q: String(q || "").slice(0, 160), text: body.slice(0, 600), conf: conf, src: src || "compose", w: 1, ts: Date.now() };
  ...
  function teach(raw) {
    ...
    if (text.length < 3) return { ok: false, msg: "Пустой факт — напишите после «запомни:»." };
    var grow = loadGrow();
  ...
  function correct(raw) {
    ...
    if (text.length < 3) return { ok: false, msg: "Укажите правильный текст после «исправь:»." };
  ...
  function reinforceLast(ok) {
    var last = null; try { last = JSON.parse(localStorage.getItem(LAST_KEY) || "null"); } catch (e) {}
    if (!last || !last.text) return { ok: false, msg: "Нет предыдущего ответа для оценки." };
    if (ok) {
    ...
    return { ok: true, msg: "Отметила ошибку. Такой ответ ослаблен. Можно «исправь: правильный текст»." };
  }
  function think(query) {
    query = String(query || "").trim();
    if (!query) return { text: "", mode: "пусто", confidence: 0, evidence: [], intent: "none", offline: true, source: "compose" };
    var intent = intentOf(query);
    if (intent === "teach") { var tr = teach(query); return { text: tr.msg, mode: "обучение", confidence: 1, intent: intent, offline: true, source: "compose", grew: tr.ok }; }
    if (intent === "correct") { var cr = correct(query); return { text: cr.msg, mode: "исправление", confidence: 1, intent: intent, offline: true, source: "compose" }; }
    if (intent === "reinforce") { var rr = reinforceLast(true); return { text: rr.msg, mode: "подкрепление", confidence: 1, intent: intent, offline: true, source: "compose" }; }
    if (intent === "punish") { var pr = reinforceLast(false); return { text: pr.msg, mode: "ослабление", confidence: 1, intent: intent, offline: true, source: "compose" }; }
    var ev = gatherEvidence(query, 10);
    var conf = confidence(ev);
    var text = generateAnswer(query, intent, ev, conf);
    bump("generations", 1);
    var grew = false;
    if (conf >= 0.42 && intent !== "greet" && intent !== "identity" && intent !== "growth_status") grew = growAbsorb(query, text, conf, "auto");
    try { localStorage.setItem(LAST_KEY, JSON.stringify({ q: query, text: text, conf: conf, ts: Date.now() })); } catch (e) {}
    var footer = "\n· уверенность ~" + Math.round(conf * 100) + "% · ядро v2 · без сети";
    if (grew) footer = "\n· факт записан в рост ядра" + footer;
    return {
      text: text + footer,
      mode: conf >= 0.2 ? "генерация" : "слабая-генерация",
      confidence: conf,
      evidence: ev.slice(0, 5).map(function (e) { return { score: e.score, src: e.src, preview: e.text.slice(0, 80) }; }),
      intent: intent, offline: true, source: "compose", arch: "Resonance-Composer-v2-ru",
      grew: grew, growthSize: loadGrow().length, version: VER
    };
  }
  function status() {
    var st = loadStats();
    return { name: "Resonance Composer", version: VER, ready: true, offline: true, growth: loadGrow().length, maxGrowth: MAX_GROW, generations: st.generations || 0, teaches: st.teaches || 0, role: "генерация + рост локального ядра" };
  }
    ...
    try {
      if (global.AKSI_NEURO && typeof global.AKSI_NEURO.think === "function" && !global.AKSI_NEURO._composeWrapped) {
        ...
        global.AKSI_NEURO.think = function (q) {
          try {
            var c = think(q);
            if (c && c.text && c.mode !== "слабая-генерация" && c.mode !== "пусто") {
              return { text: c.text, mode: c.mode, score: c.confidence, offline: true, source: "compose", arch: "Resonance-Composer-v2-ru", steps: c.growthSize || 0 };
            }
          } catch (e) {}
