/** AKSI Full System v7.0.0-llm-hybrid · aksilove@internet.ru */
(function () {
  "use strict";
  var VER = "7.0.0-llm-hybrid";
  var history = [];
  ...
  var SEED = [
    { k: ["кто ты", "что ты", "представься", "привет", "здравствуй"],
      a: "Я АКСИ — гибридный ИИ-контур в браузере.\n\nКак отвечаю:\n1) ищу факты (ядро, Neuro, Knowledge, Wikipedia);\n2) осмысливаю и собираю кандидаты;\n3) выбираю лучший (коллапс);\n4) пишу seal-след.\n\nОпционально — локальная нейросеть WebLLM (кора).\nКонтакт: aksilove@internet.ru" },
    ...
    { k: ["квантовый компьютер", "квантовые компьютер", "кубит"],
      a: "Квантовый компьютер считает на кубитах (суперпозиция 0 и 1, интерференция, запутанность).\nТеоретический выигрыш — Шор, Гровер. Практика: прототипы; отказоустойчивые машины большой мощности ещё развиваются.\nВ АКСИ «коллапс» — выбор ответа, не физический QC." },
    { k: ["искусственный интеллект", "что такое ии", "нейросет", "llm", "языков модель"],
      a: "ИИ — восприятие, вывод, обучение, генерация.\nКрупные LLM предсказывают токены на данных и зависят от облака.\nАКСИ — явный контур + локальные факты + опциональная кора, с seal, без обязательной сдачи данных." },
    { k: ["гравитац", "тяготен"],
      a: "Гравитация — притяжение масс. Ньютон: F ∝ m1·m2/r². ОТО: искривление пространства-времени." },
    { k: ["фотосинтез"], a: "Фотосинтез: 6CO₂ + 6H₂O + свет → C₆H₁₂O₆ + 6O₂. Хлоропласты, хлорофилл." },
    { k: ["днк", "ген ", "хромосом"], a: "ДНК — двойная спираль (A,T,G,C). Ген — участок ДНК. Хромосома — упаковка." },
    { k: ["блокчейн", "биткоин", "bitcoin"], a: "Блокчейн — цепочка блоков с хешами. Биткоин (2009) — криптовалюта без банка." },
    { k: ["теория относительности", "эйнштейн", "e=mc"], a: "СТО (1905): c постоянна; E=mc². ОТО (1915): гравитация — кривизна пространства-времени." },
    ...
    { k: ["мировой вопрос", "вопрос нашего времени", "главн проблем"],
      a: "Вопрос нашего времени в ИИ: сохранить пользу моделей и не отдать контроль, данные и смысл решений непрозрачным системам.\n\nАКСИ: локальный контур, видимые кандидаты, seal, опциональная кора без обязательного облака." },
    { k: ["коллапс", "суперпозиц", "как работа"], a: "Найти факты → кандидаты с весами → коллапс → seal." },
    { k: ["миссия", "зачем", "польза", "что умеешь"], a: "Полезный проверяемый ответ в браузере. Факты, синтез, выбор, память, seal, WebLLM.\naksilove@internet.ru" },
    { k: ["офлайн", "без интернет", "автоном", "sovereign"], a: "Offline-first: ядро, Neuro и память без сети. Веб и кора — усиления." }
  ];
  function seedMatch(q) {
    ...
    if (score >= 6) return { text: best.a, conf: Math.min(0.97, 0.8 + score / 40), source: "ядро" };
    return null;
  }
  ...
  async function fetchWiki(q) {
    var topic = String(q).replace(/^(что такое|кто такой|кто такая|what is|who is|расскажи про|объясни)\s+/i, "").replace(/\?+$/g, "").trim() || q;
    try {
    ...
    try {
      ...
      var s = await withTimeout(fetch(sUrl, { mode: "cors" }).then(function (res) { return res.json(); }), 3000);
  ...
  function knowledgeHit(q) {
    try {
      ...
      var r = K.search(q);
      if (r && r.body) return { text: r.title + "\n\n" + r.body, conf: 0.84, source: "knowledge", title: r.title };
    } catch (e) {}
    return null;
  }
  function neuroHit(q) {
    try {
      ...
      if (r && r.text && r.text.length > 30 && !/Спросите:|offline Neuro/i.test(r.text))
  ...
  function comprehend(q, facts) {
    ...
    var out = "Кратко по запросу «" + q + "»:\n\n" + body;
    if (f.url) out += "\n\n→ " + f.url;
    out += "\n\n(Источник: " + (f.source || "web") + (f.title ? " · " + f.title : "") + ")";
    return { text: out, conf: Math.min(0.93, (f.conf || 0.8) + 0.06), source: "синтез" };
  }
  ...
  async function cortexSynth(q, facts) {
    ...
    var ctx = (facts || []).slice(0, 4).map(function (f, i) { return i + 1 + ") " + String(f.text || "").slice(0, 420); }).join("\n");
    var hist = history.slice(-4).map(function (t) { return "Q: " + t.q + "\nA: " + String(t.a).slice(0, 200); }).join("\n");
    try {
      var r = await AKSI_WEBLLM.complete(
        (hist ? "Диалог:\n" + hist + "\n\n" : "") + "Вопрос: " + q + "\n\nФакты:\n" + (ctx || "(мало — отметь неуверенность)") + "\n\nОтвет на русском, 5–12 предложений.",
        { system: "Ты кора АКСИ. Только русский. Прямой ответ, потом пояснение. Опирайся на факты. Без воды.", temperature: 0.4, max_tokens: 560 }
      );
      var t = String((r && (r.text || r.answer)) || "").trim();
      if (t.length < 40 || !/[а-яёА-ЯЁ]/.test(t)) return null;
      return { text: t, conf: 0.95, source: "кора" };
    } catch (e) { return null; }
  }
  ...
  function loadScript(src) {
    return new Promise(function (resolve, reject) {
      ...
      s.onerror = function () { reject(new Error(src)); }; document.head.appendChild(s);
    });
  }
  ...
  function ensureModules() {
    ...
    modulesPromise = (async function () {
      try { if (!window.AKSIKnowledge && !window.AKSI_KNOWLEDGE) await loadScript("/aksi-knowledge.js?v=7").catch(function () {}); } catch (e) {}
      try { if (!window.AKSI_NEURO) await loadScript("/aksi-neuro.js?v=7").catch(function () {}); } catch (e) {}
    })();
    return modulesPromise;
  }
  async function fullThink(query, opts) {
    opts = opts || {}; query = String(query || "").trim();
    ...
    if (opts.web !== false) { var wiki = await fetchWiki(query); if (wiki) facts.push(wiki); }
    setPipe(2); status("2 · Осмысливаю…");
    var synthFacts = facts.filter(function (f) {
      var s = String(f.source || ""); return s.indexOf("wiki") === 0 || s === "knowledge";
    });
    var synth = synthFacts.length ? comprehend(query, synthFacts) : null;
    if (synth) push(synth);
    setPipe(3);
    ...
    if (!cands.length) {
      push({ text: "По «" + query + "» нет надёжного факта.\n\n• «что такое X»\n• Веб\n• Кора WebLLM\n• «запомни: …»\n\naksilove@internet.ru", conf: 0.22, source: "пробел" });
    }
    var weights = cands.map(function (c) {
      var w = c.conf || 0.5;
      if (c.source === "кора") w += 0.35;
      if (c.source === "синтез") w += 0.22;
      if (c.source === "ядро") w += 0.28;
      if (c.source === "knowledge") w += 0.14;
      if (c.source === "neuro") w += 0.12;
      if (String(c.source).indexOf("wiki") === 0) w += 0.12;
      if (c.source === "пробел") w *= 0.35;
      return w;
    });
    var amps = normalize(weights);
    setPipe(4); status("4 · Коллапс…");
    var maxI = 0; for (var i = 1; i < amps.length; i++) if (amps[i] > amps[maxI]) maxI = i;
    var idx = amps[maxI] >= 0.3 ? maxI : collapse(amps);
    var chosen = cands[idx];
    setPipe(5); status("");
    var h = sealPush(query, chosen.text, chosen.source, amps[idx]);
    history.push({ q: query, a: chosen.text }); if (history.length > 12) history = history.slice(-12);
    return {
      ok: true, answer: chosen.text, source: chosen.source, probability: +amps[idx].toFixed(4),
      ...
      seal: h, cortex: cortexReady(), version: VER
    };
  }
  var lastSup = [];
  function renderSup() {
  ...
  function renderSeals() {
    ...
    el.innerHTML = list.slice(0, 25).map(function (s) {
      return '<li><span class="trail">' + s.h + "</span> · " + (s.src || "") + " · P=" + ((s.p || 0) * 100).toFixed(0) + "%<br>" + String(s.q || "").replace(/</g, "<") + '<div class="meta">' + new Date(s.t).toLocaleString() + "</div></li>";
    }).join("");
  }
  function renderSys() {
  ...
  async function ask(raw) {
    ...
    try {
      ...
      lastSup = r.superposition || []; renderSup(); renderSeals();
    } catch (e) {
      if ($("out")) $("out").textContent = "Ошибка: " + (e.message || e);
    } finally {
      if ($("go")) $("go").disabled = false; status("");
    }
  }
  ...
  async function ensureCortex() {
    ...
    if (!window.AKSI_WEBLLM) await loadScript("/aksi-webllm.js?v=7");
  ...
  function boot() {
    ...
    if ($("memSave")) $("memSave").onclick = function () {
      ...
      if ($("memIn")) $("memIn").value = ""; showTab("mem");
    };
    ...
    if ($("cortexBtn")) $("cortexBtn").onclick = async function () {
      ...
      try { var ok = await ensureCortex(); btn.textContent = ok ? "Кора ON" : "Кора сбой"; }
