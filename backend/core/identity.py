...
Private keys are never persisted by this module. The host application must load
them from a secure local secret store; public keys may be published safely.
"""
...
def sign(value: Any, private_key_b64: str) -> str:
    ...
    return base64.urlsafe_b64encode(
        Ed25519PrivateKey.from_private_bytes(raw).sign(canonical_json(value))
    ).decode().rstrip("=")
