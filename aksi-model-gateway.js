(function(){
  ...
  function bridge(){const w=window.AKSI_WEBLLM;if(!w)return;register({id:'webllm',kind:'llm',runtime:'WebLLM/WebGPU',local:true,capabilities:['chat','generation']},(input,options)=>w.complete(input,options));window.dispatchEvent(new CustomEvent('aksi:gateway-bridged',{detail:health()}))}
  window.AKSIModeGateway={version:'1.2',register,list,infer,health,bridge};
