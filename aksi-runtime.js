/* AKSI Runtime v6 — one browser runtime. Local-first; network is explicit opt-in; proof is integrity, not truth. */
(function(g){'use strict';
const VERSION='6.0.0', enc=new TextEncoder();
...
async function boot(){
 ...
 const loaded=await loadScript('/aksi-core.js');
 const core=g.AksiCore||null;
 const runtime={runtimeVersion:VERSION,core,events,localFirst:true,
   async ask(input){const q=String(input||'').trim();if(!q)return{schema:'AKSI-RESULT-1',status:'error',error:'empty-input'};emit('request',{input:q});let r;if(core&&typeof core.reply==='function'){try{r=await core.reply(q)}catch(e){r={text:'АКСИ не смогла выполнить запрос.',source:'runtime',status:'error',error:String(e.message||e)}}}else r={text:'AKSI Core недоступен. Внешний ответ не имитируется.',source:null,status:'unverified'};const proof=r.event?{hash:r.event.event_hash,previousHash:r.event.previous_hash,schema:r.event.schema,type:r.event.type}:null;const out={schema:'AKSI-RESULT-1',id:crypto.randomUUID?crypto.randomUUID():Date.now().toString(36),input:q,answer:String(r.text||''),source:r.source||null,status:r.status||'unverified',proof,timestamp:new Date().toISOString(),networkEnabled:!!core?.webEnabled?.()};emit('answer',out);return out},
   async status(){let self=null;try{self=core&&core.selfTest?await core.selfTest():null}catch(e){self={ok:false,error:String(e.message||e)}}const identity=self&&self.identity&&self.identity!=='unavailable';return{runtime:VERSION,loadedCore:loaded,online:navigator.onLine,localFirst:true,webEnabled:!!core?.webEnabled?.(),modules:{core:!!core,crypto:!!g.crypto?.subtle,indexedDB:!!g.indexedDB,webAssembly:!!g.WebAssembly,proof:!!core?.verify,memory:!!core?.memory,identity:identity},selfTest:self,eventCount:events.length}},
