/**
 * AKSI WebLLM runtime v1.8.1-ru — WebGPU (MLC) + WASM + полный русский system prompt
 * Hardened: multi-CDN, 45s timeout, max_tokens 512, visible RU answers
 * © AKSI · aksilove@internet.ru
 */
(function (G) {
  "use strict";

  var VERSION = "1.8.1-webllm-ru";
  ...
  var MODELS = [
    ...
    { id: "Xenova/LaMini-Flan-T5-248M", label: "LaMini WASM (без WebGPU)", backend: "transformers" },
  ...
  var engine = null, xfPipe = null, backend = null, currentModel = null;
  var loading = false, progress = 0, message = "не загружено", lastError = null;
  var webgpu = null, loadPromise = null, bootstrapNetworkUsed = false;

  function status() {
    return {
      version: VERSION, webllm: WEBLLM_VERSION, ready: !!(engine || xfPipe),
      loading: loading, progress: progress, message: message, model: currentModel,
      backend: backend, webgpu: webgpu, error: lastError, models: MODELS.slice(),
      inference_local: !!(engine || xfPipe), network_required_for_bootstrap: bootstrapNetworkUsed
    };
  }
  function emit() {
  ...
  function setProgress(p, msg) {
    if (typeof p === "number" && !isNaN(p)) progress = Math.max(0, Math.min(100, Math.round(p <= 1 ? p * 100 : p)));
  ...
  function detectWebGPU() {
    ...
    return G.navigator.gpu.requestAdapter().then(function (a) { webgpu = !!a; return webgpu; })
  ...
  function importFirst(urls) {
    var last = null, i = 0;
    function next() {
      if (i >= urls.length) return Promise.reject(last || new Error("CDN недоступен. Проверьте сеть и блокировщики."));
      var u = urls[i++];
      bootstrapNetworkUsed = true;
      setProgress(Math.min(12, 2 + i * 2), "runtime… " + u.replace(/^https:\/\//, "").slice(0, 40));
      return importWithTimeout(u, 45000).catch(function (e) { last = e; return next(); });
    }
    return next();
  }
  function extractCreateEngine(mod) {
    ...
    return typeof mod.createEngine === "function" ? mod.createEngine : null;
  }
  ...
  function loadWebLLM(modelId, onProgress) {
    ...
    return detectWebGPU().then(function (ok) {
      if (!ok) throw new Error("WebGPU недоступен — используйте «Только WASM»");
      ...
      return createEngine(modelId, {
        initProgressCallback: function (info) {
          ...
          setProgress(p, (info && info.text) ? String(info.text).slice(0, 160) : "загрузка WebLLM…");
          if (typeof onProgress === "function") onProgress(status());
        }
      });
    }).then(function (instance) {
      engine = instance; xfPipe = null; backend = "webllm"; currentModel = modelId; lastError = null;
  ...
  function loadTransformers(modelId, onProgress) {
    ...
    return importFirst(XF_CDNS).then(function (mod) {
      ...
      try {
        if (mod.env) { mod.env.allowLocalModels = false; mod.env.useBrowserCache = true; mod.env.allowRemoteModels = true; }
        if (mod.default && mod.default.env) { mod.default.env.allowLocalModels = false; mod.default.env.useBrowserCache = true; mod.default.env.allowRemoteModels = true; }
      } catch (e) {}
      ...
      return pipeline(task, modelId, {
        progress_callback: function (info) {
          ...
          if (info && typeof info.progress === "number") {
            var pr = info.progress; if (pr > 1) pr = pr / 100;
            p = 12 + Math.max(0, Math.min(1, pr)) * 85;
          }
          var msg = (info && info.status) ? String(info.status) : "скачивание WASM…";
      ...
      xfPipe = pipe; engine = null; backend = "transformers"; currentModel = modelId; lastError = null;
  ...
  function loadModel(modelId, onProgress, opts) {
    ...
    if (typeof modelId === "object" && modelId !== null && !Array.isArray(modelId)) {
      opts = modelId; modelId = opts.model || opts.modelId || null; onProgress = opts.onProgress || onProgress;
    }
    if (typeof onProgress === "object" && onProgress !== null && onProgress.forceWasm != null) {
      opts = onProgress; onProgress = opts.onProgress || null;
    }
    if (loading && loadPromise) return loadPromise;
    loading = true; progress = 0; lastError = null; bootstrapNetworkUsed = false;
    message = "старт загрузки…"; emit();
    if (typeof onProgress === "function") onProgress(status());
    ...
    loadPromise = detectWebGPU().then(function (gpu) {
      if (forceWasm || !gpu) {
        setProgress(3, forceWasm ? "режим WASM" : "нет WebGPU → WASM");
        return loadTransformers(preferred && /Xenova|distilgpt/i.test(preferred) ? preferred : "Xenova/LaMini-Flan-T5-248M", onProgress);
      }
      ...
      return loadWebLLM(preferred || MODELS[0].id, onProgress).catch(function (webllmError) {
        ...
        return loadTransformers("Xenova/LaMini-Flan-T5-248M", onProgress).catch(function (xfError) {
          lastError = first + " | WASM: " + String((xfError && xfError.message) || xfError);
          throw new Error(lastError);
        });
      });
    }).then(function (st) { loading = false; loadPromise = null; return st; })
      .catch(function (error) {
        loading = false; loadPromise = null;
        lastError = lastError || String((error && error.message) || error);
        message = "ошибка: " + lastError.slice(0, 180); progress = 0; emit();
        if (typeof onProgress === "function") onProgress(status());
        throw error;
      });
    return loadPromise;
  ...
  function unload() {
    engine = null; xfPipe = null; backend = null; currentModel = null;
    loading = false; loadPromise = null; progress = 0; lastError = null; message = "выгружено";
  ...
  function complete(prompt, options) {
    ...
    var system = options.system ||
      ("Ты — АКСИ, локальный ИИ-ассистент. Всегда отвечай на русском языке, ясно и по делу. " +
       "Не выдумывай факты. Если не знаешь — скажи прямо. Не упоминай другие ИИ-компании. " +
       "Стиль: спокойный, точный, полезный. Формула продукта: AKSI=(A×I×S)×(1+0.4√n).");
    ...
    if (backend === "transformers" && xfPipe) {
      ...
      var input = isT5
        ...
        : ("<|im_start|>system\nТы АКСИ. Отвечай только на русском, полно и ясно.<|im_end|>\n<|im_start|>user\n" + q + "<|im_end|>\n<|im_start|>assistant\n");
      return Promise.resolve().then(function () {
        return xfPipe(input, { max_new_tokens: Math.min(320, maxTokens), temperature: temperature, do_sample: temperature > 0.05 });
    ...
    return Promise.resolve({
      ...
      error: "модель не загружена — нажмите «Только WASM» или «Загрузить WebLLM»",
      inference_local: false, bootstrap_network_used: bootstrapNetworkUsed, model: currentModel, backend: backend
    });
  }
  G.AKSI_WEBLLM = {
    version: VERSION, status: status, load: loadModel, autoLoad: autoLoad, unload: unload,
    complete: complete, think: complete, ask: complete, models: MODELS,
