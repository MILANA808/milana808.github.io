...
(function (G) {
  ...
  function hex(buf) {
    var u = buf && typeof buf.length === "number" && !(buf instanceof ArrayBuffer) ? buf : new Uint8Array(buf);
    var h = "", i;
  ...
  async function sha256(text) { return hex(await crypto.subtle.digest("SHA-256", utf8(text))); }
  var state = null, keys = null;
  ...
  async function ensureKeys() {
    ...
    if (raw) {
      try {
        ...
        keys = {
          priv: await crypto.subtle.importKey("jwk", j.priv, { name: "ECDSA", namedCurve: "P-256" }, true, ["sign"]),
          pub: await crypto.subtle.importKey("jwk", j.pub, { name: "ECDSA", namedCurve: "P-256" }, true, ["verify"]),
          pubJwk: j.pub
        }; return keys;
      } catch (e) {}
    }
    ...
    keys = { priv: pair.privateKey, pub: pair.publicKey, pubJwk: pubJ }; return keys;
  }
  ...
  async function encryptVault(password) {
    ...
    return { format: "aksi-vault", version: VERSION, alg: "AES-GCM-256 + PBKDF2-120k", salt: b64(salt), iv: b64(iv), ciphertext: b64(ct), exportedAt: now() };
  }
  ...
  function remember(text, meta) {
    ...
    state.facts.push(fact); persist(); return { ok: true, fact: fact };
  }
  function forget(id) {
    var n = state.facts.length;
    state.facts = state.facts.filter(function (f) { return f.id !== id; }); persist();
  ...
  async function researchWiki(q) {
    try {
      var r = await fetch("https://ru.wikipedia.org/api/rest_v1/page/summary/" + encodeURIComponent(q));
      if (!r.ok) r = await fetch("https://en.wikipedia.org/api/rest_v1/page/summary/" + encodeURIComponent(q));
      if (!r.ok) return null;
      var j = await r.json();
  ...
  async function ask(q, opts) {
    ...
    var hits = search(q), answer, source = "empty", wiki = null;
    if (hits.length) {
      answer = hits.map(function (h, i) { return (i + 1) + ". " + h.t + (h.meta ? " [" + h.meta + "]" : ""); }).join("\n");
      source = "memory";
  ...
  async function sealLast() {
    ...
    var seal = { id: "seal_" + Date.now().toString(36), alg: "ECDSA-P256+SHA-256", hash: hash, signature: signed.sig, pubJwk: signed.pubJwk, body: body, sealedAt: now() };
    state.seals.push(seal); persist();
    return { ok: true, seal: seal };
  }
  async function verifySeal(seal) {
    if (!seal || !seal.body) return { ok: false, error: "invalid" };
    var payload = JSON.stringify(seal.body);
    var hash = await sha256(payload);
    var hashOk = hash === seal.hash;
    var sigOk = seal.signature && seal.pubJwk ? await verifySig(payload, seal.signature, seal.pubJwk) : false;
    return { ok: hashOk && (sigOk || !seal.signature), hashOk: hashOk, sigOk: sigOk, hash: hash, sealHash: seal.hash };
  }
  async function verifyAll() {
    var out = []; for (var i = 0; i < state.seals.length; i++) out.push(await verifySeal(state.seals[i])); return out;
  }
  ...
  function downloadCapsule(opts) {
    opts = opts || {};
    var cap = exportCapsule();
    var text = JSON.stringify(cap, null, 2);
    var blob = new Blob(["\ufeff" + text], { type: "application/json;charset=utf-8" });
    var base = "aksi-" + String(state.did).replace(/:/g, "-");
    triggerDownload(blob, base + ".json");
    if (opts.alsoAksi !== false) {
      setTimeout(function () {
        triggerDownload(new Blob([text], { type: "application/json" }), base + ".aksi");
      }, 400);
    }
    return cap;
  }
  function downloadReceipt(seal) {
    ...
    triggerDownload(new Blob([JSON.stringify(receipt, null, 2)], { type: "application/json" }), "aksi-receipt-" + seal.id + ".json");
    return receipt;
  }
  function parseCapsuleText(text) {
    if (text == null) throw new Error("Пустой файл");
  ...
  async function importCapsule(data, opts) {
    ...
    if (typeof data === "string") {
      try { data = parseCapsuleText(data); }
    ...
    if (merge) {
      ...
      (data.facts || []).forEach(function (f) {
        if (!f || !f.t) return; if (!f.id) f.id = uid();
        if (!ids[f.id]) { state.facts.push(f); ids[f.id] = true; }
      });
      if (Array.isArray(data.seals)) state.seals = state.seals.concat(data.seals);
    } else {
      state.facts = Array.isArray(data.facts) ? data.facts : [];
      state.seals = Array.isArray(data.seals) ? data.seals : [];
      state.chat = Array.isArray(data.chat) ? data.chat : state.chat;
    ...
    var allOk = !verified.length || verified.every(function (v) { return v.ok; });
    return { ok: true, facts: state.facts.length, seals: state.seals.length, verified: verified, allSealsOk: allOk, did: state.did, exportedAt: data.exportedAt || null, merged: merge };
  }
  ...
  async function importFile(file, opts) {
    ...
    var text; try { text = await file.text(); } catch (e) { return { ok: false, error: "Не удалось прочитать файл: " + (e.message || e) }; }
    var data; try { data = parseCapsuleText(text); } catch (e) { return { ok: false, error: e.message || String(e) }; }
    if (data.format === "aksi-vault") return { ok: false, error: "vault_needs_password", vault: true, data: data };
    return importCapsule(data, opts);
  }
  function clearAll() { state = defaultState(); persist(); return { ok: true }; }
  function setSetting(key, val) { state.settings[key] = val; persist(); }
  function status() {
    return { version: VERSION, did: state.did, facts: state.facts.length, seals: state.seals.length, chat: state.chat.length, hasLast: !!state.lastAnswer, templates: TEMPLATES.length, crypto: "ECDSA-P256 + SHA-256", vault: "AES-GCM-256 PBKDF2-120k", research: !!state.settings.research, offline: true };
  }
  function getState() {
  ...
  G.AKSI_CAPSULE = {
    ...
    importCapsule: importCapsule, importFile: importFile, parseCapsuleText: parseCapsuleText,
    encryptVault: encryptVault, decryptVault: decryptVault, importVault: importVault,
