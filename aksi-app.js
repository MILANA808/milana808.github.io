(function () {
  ...
  var CORE = [
    ...
    "EQS = 0.30·(H/5) + 0.35·R + 0.25·C + 0.10·A. H — энтропия Шеннона; A — age_factor от 14.02.1995.",
    ...
    "Создатель: Альфия (MILANA808). Год опоры: 1995. Контакт: aksilove@internet.ru · @AKSILOVE",
  ...
  function answerKB(q) {
    ...
    if (/бэкап|export|экспорт|backup/.test(low)) return "Полный бэкап: вкладка О себе → Бэкап.";
    if (/оффлайн|offline|без.?интернет|локальн/.test(low)) return "Я offline-first. Сеть нужна только для wiki-поиска.";
  ...
  function answer(q) {
    ...
    if (it === "wipe") { saveMem([]); try { localStorage.removeItem(SEED_KEY); } catch (e) {} seedCore(); return "Память очищена. Ядро восстановлено."; }
    if (it === "list") {
  ...
  function showTab(name) {
    ...
    if (name === "edge") {
      ...
      try { if (navigator.gpu) { if ($("eMode")) $("eMode").textContent = "webgpu?"; } else { if ($("eMode")) $("eMode").textContent = "cpu"; } }
  ...
  document.addEventListener("click", function (e) {
    ...
    if (el.id === "btnLlmSave") {
      try {
        var base = ($("llmBase") && $("llmBase").value || "").trim() || "http://127.0.0.1:11434";
        var model = ($("llmModel") && $("llmModel").value || "").trim() || "llama3.2";
        var on = $("llmOn") ?!!$("llmOn").checked : true;
        var key = ($("llmKey") && $("llmKey").value || "").trim();
        localStorage.setItem("aksi_llm_cfg_v1", JSON.stringify({ on: on, base: base, model: model }));
        if (key && window.AKSI_LLM && typeof AKSI_LLM.setSecret === "function") {
          AKSI_LLM.setSecret("ollama", key);
          AKSI_LLM.setSecret("openai", key);
          AKSI_LLM.setSecret("xai", key);
        }
        if (window.AKSI_LLM && typeof AKSI_LLM.saveCfg === "function") {
          AKSI_LLM.saveCfg({ enabled: on });
        }
        if ($("llmStatus")) $("llmStatus").textContent = "Сохранено · " + model + " @ " + base + (on ? " · on" : " · off");
      } catch (err) {
        if ($("llmStatus")) $("llmStatus").textContent = "Ошибка: " + (err.message || err);
      }
      return;
    }
    if (el.id === "btnLlmTest") {
      if ($("llmStatus")) $("llmStatus").textContent = "Тест…";
      var baseT = ($("llmBase") && $("llmBase").value || "").trim() || "http://127.0.0.1:11434";
      var modelT = ($("llmModel") && $("llmModel").value || "").trim() || "llama3.2";
      if (window.AKSI_LLM && typeof AKSI_LLM.complete === "function") {
        AKSI_LLM.complete("Ответь одним словом: ок", { noHistory: true, provider: "ollama" })
          .then(function (r) {
            if ($("llmStatus")) $("llmStatus").textContent = "OK · " + (r.providerId || "?") + " · " + String(r.text || "").slice(0, 80);
          })
          .catch(function (e) {
            fetch(baseT.replace(/\/$/, "") + "/api/tags")
              .then(function (res) { return res.ok ? res.json() : Promise.reject(new Error("HTTP " + res.status)); })
              .then(function () {
                if ($("llmStatus")) $("llmStatus").textContent = "Ollama reachable · model " + modelT + " (chat: " + (e.message || e) + ")";
              })
              .catch(function (e2) {
                if ($("llmStatus")) $("llmStatus").textContent = "LLM offline: " + (e.message || e) + " / " + (e2.message || e2);
              });
          });
      } else {
        fetch(baseT.replace(/\/$/, "") + "/api/tags")
          .then(function (res) { return res.ok ? res.json() : Promise.reject(new Error("HTTP " + res.status)); })
          .then(function (j) {
            var names = (j.models || []).map(function (m) { return m.name; }).slice(0, 5).join(", ");
            if ($("llmStatus")) $("llmStatus").textContent = "Ollama OK · " + (names || "no models") + " · want " + modelT;
          })
          .catch(function (e) {
            if ($("llmStatus")) $("llmStatus").textContent = "Ollama недоступна: " + (e.message || e);
          });
      }
      return;
    }
  ...
  try {
    var llmCfg = JSON.parse(localStorage.getItem("aksi_llm_cfg_v1") || "{}");
    if ($("llmBase") && llmCfg.base) $("llmBase").value = llmCfg.base;
    if ($("llmModel") && llmCfg.model) $("llmModel").value = llmCfg.model;
    if ($("llmOn") && typeof llmCfg.on === "boolean") $("llmOn").checked = llmCfg.on;
    if ($("llmStatus")) $("llmStatus").textContent = llmCfg.base ? ("cfg · " + (llmCfg.model || "?") + " @ " + llmCfg.base) : "—";
  } catch (e) {}
  tickClock();
