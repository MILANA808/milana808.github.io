/**
 * AKSI ONE v1.8.2 — max chat runtime
 * Routes think() → MIND → LIVE → local.
...
(function (global) {
  ...
  function localAnswer(q) {
    ...
    if (/^(привет|здравств|добрый|hello|hi)\b/.test(low))
      return "Привет! Я АКСИ — помощник в вашем браузере.";
    if (/кто ты|что ты такое|what are you/.test(low))
      return "Я АКСИ — локальный помощник. Данные остаются у вас. Контакт: aksilove@internet.ru";
    if (/что умеешь|помощь|help|функци/.test(low))
      return "Чат, память («запомни: …»), фото OCR, Ollama. Вкладка «Помощь».";
  ...
  function tryOllama(q) {
    ...
    return fetch(cfg.base + "/api/generate", {
      ...
      body: JSON.stringify({ model: cfg.model, stream: false, prompt: "Ты АКСИ. Кратко.\nВопрос: " + q, options: { temperature: 0.7 } })
  ...
  function thinkLocal(q) {
    ...
    if (/^\/demo\b/i.test(q) || /^demo$/i.test(q)) {
      return Promise.resolve({ text: "AKSI demo: whoami · запомни: · Lab → ADIA/Neuro", meta: "demo" });
    }
    if (/^whoami$/i.test(q)) {
      var did = "—";
      try { did = localStorage.getItem("aksi_did_v1") || "did:aksi:local"; } catch (e) {}
      return Promise.resolve({ text: "DID: " + did + "\nlocal-first", meta: "identity" });
    }
    ...
    return tryBrain(q).then(function (br) {
      ...
      return tryCore(q).then(function (coreRes) {
        ...
        return tryLLM(q).then(function (llm) {
          ...
          return { text: "Пока нет точного ответа. Напишите «запомни: факт» или Lab → Neuro.", meta: "fallback" };
        });
      });
    });
  }
  function think(q) {
    q = String(q || "").trim();
    if (!q) return Promise.resolve({ text: "Пустой запрос.", meta: "one" });
    if (global.AKSI_MIND && typeof global.AKSI_MIND.think === "function" && !think._viaMind) {
      try {
        return global.AKSI_MIND.think(q).then(function (r) {
