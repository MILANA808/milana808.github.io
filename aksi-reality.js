/**
 * AKSI Reality Layer v0.2
 * Browser observations → auditable RealityEvents (observe-only).
 * Opt-in capabilities. No autonomous actions.
...
(function (G) {
  ...
  function caps() {
    return Object.assign({}, DEFAULTS, load(CAP_KEY, {}));
  }
  ...
  function baseEvent(kind, source, observation, confidence, evidence) {
    ...
    event.cryptographic_seal = {
      kind: "fnv",
      fnv: fnv(JSON.stringify({
        id: event.id,
        kind: event.kind,
        source: event.source,
        ...
        function (p) {
          resolve(
            baseEvent(
              ...
              p.coords.accuracy
                ? Math.max(0, Math.min(1, 1 / (1 + p.coords.accuracy / 50)))
                : 0.5,
  ...
  async function requestMedia(kind) {
    ...
    var event = baseEvent(
      "capability-grant",
      kind,
      { tracks: stream.getTracks().map(function (t) { return t.kind; }) },
      1,
  ...
  async function networkProbe() {
    ...
    return baseEvent(
      "observation",
      "network",
      { online: online, head_ok: ok, rtt_ms: Date.now() - t0 },
      online ? 0.8 : 0.3,
      ["navigator.onLine", "fetch-head"]
    );
  }

  function worldState() {
    return {
      schema: "aksi-world-state-1",
      ts: new Date().toISOString(),
      capabilities: caps(),
      event_count: listEvents().length,
      observe_only: true,
      last_events: listEvents().slice(-5)
    };
  }
  ...
  function enrichDecision(decision) {
    ...
    decision.reality = {
      ...
      event_count: listEvents().length,
      capabilities: caps(),
      recent: ev.map(function (e) {
        return {
          id: e.id,
          kind: e.kind,
          source: e.source,
          confidence: e.confidence,
          seal: e.cryptographic_seal
        };
      })
    };
    return decision;
